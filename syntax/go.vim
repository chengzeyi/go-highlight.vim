" Copyright 2020 Cheng Zeyi <ichengzeyi@gmail.com>. All rights reserved.
" Developed based on the go.vim plugin.
...
function! s:has(n) abort
    return get(g:, 'go_highlight_' . a:n, 1)
endfunction
...
" Spaces after "[]"
if s:has('array_white_space_error')
    syn match goSpaceError display "\%(\[\]\)\@<=\s\+"
endif

" Spacing errors around the 'chan' keyword
if s:has('chan_white_space_error')
...
" Space-tab error
if s:has('space_tab_error')
    syn match goSpaceError display " \+\t"me=e-1
endif

" Trailing white space error
if s:has('trailing_white_space_error')
...
if s:has('variable_assignments')
    syn match goVarAssign /\v[_.[:alnum:]]+(,\s*[_.[:alnum:]]+)*\ze(\s*([-^+|^\/%&]|\*|\<\<|\>\>|\&\^)?\=[^=])/
    hi def link   goVarAssign         Special
endif

" Variable Declarations
if s:has('variable_declarations')
    syn match goVarDefs /\v\w+(,\s*\w+)*\ze(\s*:\=)/
    hi def link   goVarDefs           Special
endif

" Build Constraints
if s:has('build_constraints')
...
if s:has('build_constraints') || s:has('fold_package_comment')
    ...
    exe 'syn region  goPackageComment    start=/\v(\/\/.*\n)+\s*package/'
                ...
                \ . (s:has('fold_package_comment') ? ' fold' : '')
